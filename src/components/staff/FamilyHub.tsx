...
import { Baby, BarChart3, BookOpenCheck, Brain, Family, Loader2, Plus, ShieldCheck, Trash2 } from 'lucide-react'
...
export function FamilyHub({ user }: { user: any }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (/staff_children|does not exist|schema cache|PGRST205/i.test(raw)) {
        setError('O módulo Família ainda precisa do SQL de ativação no Supabase: supabase/migrations/20260819_staff_family_studies_v1.sql')
      } else {
        setError(raw || 'Não consegui carregar o módulo Família.')
      }
    } finally {
      setLoading(false)
    }
  ...
  async function removeChild(child: StaffChild) {
    ...
    try {
      ...
      setMaterials((current) => current.filter((item) => item.child_id !== child.id))
      setGameSessions((current) => current.filter((item) => item.child_id !== child.id))
      setStudyAttempts((current) => current.filter((item) => item.child_id !== child.id))
    } catch (reason) {
  ...
  const childStats = useMemo(() => children.map((child) => {
    ...
    const gameQuestions = games.reduce((sum, item) => sum + item.total_questions, 0)
    const gameScore = games.reduce((sum, item) => sum + item.score, 0)
    const studyQuestions = attempts.reduce((sum, item) => sum + item.total_questions, 0)
    const studyScore = attempts.reduce((sum, item) => sum + item.score, 0)
    const totalQuestions = gameQuestions + studyQuestions
    const totalScore = gameScore + studyScore
    const accuracy = totalQuestions ? Math.round((totalScore / totalQuestions) * 100) : 0
    const minutes = Math.round(games.reduce((sum, item) => sum + item.duration_seconds, 0) / 60)
    return { child, games: games.length, materials: childMaterials.length, attempts: attempts.length, accuracy, minutes }
    ...
    <div className="max-w-6xl mx-auto">
      <section className="rounded-[30px] border border-purple-500/20 bg-gradient-to-br from-purple-500/10 via-slate-950/60 to-fuchsia-500/5 p-5 md:p-7">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex gap-4 items-start"><div className="w-14 h-14 rounded-2xl bg-purple-500/15 grid place-items-center text-purple-300"><Family className="w-7 h-7" /></div><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-purple-300">Staff Família</p><h1 className="text-2xl md:text-3xl font-black mt-1">Um staff para a rotina dos filhos também.</h1><p className="text-slate-500 mt-2 max-w-2xl">Perfis gerenciados pelo responsável, tempo de tela educativo, materiais escolares e acompanhamento de estudos — sem conta infantil independente.</p></div></div>
      ...
      {tab === 'children' && (
        <div className="grid lg:grid-cols-[1fr_380px] gap-5">
          <section className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5 md:p-6">
            <div className="flex items-center justify-between"><div><h2 className="text-xl font-black">Perfis dos filhos</h2><p className="text-sm text-slate-500 mt-1">Use apelido ou primeiro nome; não precisamos da data de nascimento.</p></div><span className="text-xs text-slate-600">{children.length} perfil{children.length === 1 ? '' : 'is'}</span></div>
            <div className="grid sm:grid-cols-2 gap-3 mt-5">{children.map((child) => <article key={child.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4"><div className="flex items-start gap-3"><div className="w-12 h-12 rounded-2xl bg-purple-500/10 grid place-items-center text-2xl">{child.avatar_emoji}</div><div className="flex-1 min-w-0"><h3 className="font-black truncate">{child.display_name}</h3><p className="text-xs text-purple-300 mt-1">{child.age_group} anos{child.school_grade ? ` · ${child.school_grade}` : ''}</p></div><button onClick={() => void removeChild(child)} className="p-2 text-slate-700 hover:text-rose-300"><Trash2 className="w-4 h-4" /></button></div></article>)}{!children.length && <div className="sm:col-span-2 rounded-2xl border border-dashed border-slate-800 p-8 text-center text-slate-600">Cadastre o primeiro perfil para liberar Desafios Kids e Estudos.</div>}</div>
          </section>
      ...
      {tab === 'progress' && (
        <div className="space-y-4">
          {childStats.map(({ child, games, materials: countMaterials, attempts, accuracy, minutes }) => <section key={child.id} className="rounded-3xl border border-slate-800 bg-slate-950/60 p-5 md:p-6"><div className="flex flex-col md:flex-row md:items-center gap-5"><div className="flex items-center gap-3 md:w-64"><div className="w-14 h-14 rounded-2xl bg-purple-500/10 grid place-items-center text-2xl">{child.avatar_emoji}</div><div><h3 className="font-black text-lg">{child.display_name}</h3><p className="text-xs text-slate-500">{child.school_grade || `${child.age_group} anos`}</p></div></div><div className="grid grid-cols-2 sm:grid-cols-4 gap-3 flex-1"><div className="rounded-2xl bg-slate-900 border border-slate-800 p-3"><p className="text-2xl font-black">{games}</p><p className="text-xs text-slate-500">sessões Kids</p></div><div className="rounded-2xl bg-slate-900 border border-slate-800 p-3"><p className="text-2xl font-black">{minutes}</p><p className="text-xs text-slate-500">minutos de desafios</p></div><div className="rounded-2xl bg-slate-900 border border-slate-800 p-3"><p className="text-2xl font-black">{countMaterials}</p><p className="text-xs text-slate-500">materiais de estudo</p></div><div className="rounded-2xl bg-slate-900 border border-slate-800 p-3"><p className="text-2xl font-black">{attempts ? `${accuracy}%` : '—'}</p><p className="text-xs text-slate-500">acertos registrados</p></div></div></div></section>)}
